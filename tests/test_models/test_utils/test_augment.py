...
augment_cfgs = [
    dict(type="BatchCutMix", alpha=1.0, prob=1.0),
    dict(type="BatchMixup", alpha=1.0, prob=1.0),
    dict(type="Identity", prob=1.0),
    dict(type="BatchResizeMix", alpha=1.0, prob=1.0),
]
...
def test_augments():
    ...
    # Test cutmix
    augments_cfg = dict(type="BatchCutMix", alpha=1.0, num_classes=10, prob=1.0)
    ...
    # Test mixup
    augments_cfg = dict(type="BatchMixup", alpha=1.0, num_classes=10, prob=1.0)
    augs = Augments(augments_cfg)
    mixed_imgs, mixed_labels = augs(imgs, labels)
    assert mixed_imgs.shape == torch.Size((4, 3, 32, 32))
    assert mixed_labels.shape == torch.Size((4, 10))
    ...
    # Test resizemix
    augments_cfg = dict(
        type="BatchResizeMix", alpha=1.0, num_classes=10, prob=1.0
    )
    augs = Augments(augments_cfg)
    mixed_imgs, mixed_labels = augs(imgs, labels)
    assert mixed_imgs.shape == torch.Size((4, 3, 32, 32))
    ...
    augments_cfg = [
        ...
        dict(type="BatchMixup", alpha=1.0, num_classes=10, prob=0.3),
    ]
    augs = Augments(augments_cfg)
    mixed_imgs, mixed_labels = augs(imgs, labels)
    assert mixed_imgs.shape == torch.Size((4, 3, 32, 32))
    ...
    augments_cfg = [
        dict(type="BatchCutMix", alpha=1.0, num_classes=10, prob=0.5),
        dict(type="BatchMixup", alpha=1.0, num_classes=10, prob=0.5),
    ]
    augs = Augments(augments_cfg)
    mixed_imgs, mixed_labels = augs(imgs, labels)
    assert mixed_imgs.shape == torch.Size((4, 3, 32, 32))
