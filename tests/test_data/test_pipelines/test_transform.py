...
import pepper.datasets.pipelines.transforms as mmcls_transforms
from pepper.datasets.builder import PIPELINES
from pepper.datasets.pipelines import Compose
...
def test_resize():
    ...
    transform_cv2 = dict(
        type="Resize", size=(224, 256), interpolation="bilinear", backend="cv2"
    )
    transform_pil = dict(
        type="Resize",
        size=(224, 256),
...
def test_randomcrop():
    ...
    for mode in ["constant", "edge", "reflect", "symmetric"]:
        ...
        kwargs = dict(size=(500, 600), padding=0, pad_if_needed=True, pad_val=0)
        random.seed(seed)
        np.random.seed(seed)
        aug = []
        aug.extend([mmcls_transforms.RandomCrop(**kwargs)])
...
def test_randomresizedcrop():
    ...
    with pytest.raises(ValueError):
        kwargs = dict(
            size=(200, 300), scale=(1.0, 0.08), ratio=(3.0 / 4.0, 4.0 / 3.0)
        )
        aug = []
    ...
    with pytest.raises(ValueError):
        kwargs = dict(
            size=(200, 300), scale=(0.08, 1.0), ratio=(4.0 / 3.0, 3.0 / 4.0)
        )
        aug = []
        aug.extend([mmcls_transforms.RandomResizedCrop(**kwargs)])
        composed_transform = Compose(aug)
    ...
    # test crop size < image size
    kwargs = dict(
        size=(200, 300), scale=(0.08, 1.0), ratio=(3.0 / 4.0, 4.0 / 3.0)
    )
    random.seed(seed)
    np.random.seed(seed)
    aug = []
    ...
    # test crop size > image size
    kwargs = dict(
        size=(600, 700), scale=(0.08, 1.0), ratio=(3.0 / 4.0, 4.0 / 3.0)
    )
    random.seed(seed)
    np.random.seed(seed)
    aug = []
...
def test_random_erasing():
    ...
    with pytest.raises(AssertionError):
        cfg = dict(type="RandomErasing", erase_prob=-1.0)
    ...
    with pytest.raises(AssertionError):
        cfg = dict(type="RandomErasing", min_area_ratio=-1.0)
        build_from_cfg(cfg, PIPELINES)
    with pytest.raises(AssertionError):
    ...
    with pytest.raises(AssertionError):
        ...
        cfg = dict(type="RandomErasing", min_area_ratio=0.6, max_area_ratio=0.4)
        build_from_cfg(cfg, PIPELINES)
    ...
    assert random_erasing.aspect_range == (0.5, 2.0)

    cfg = dict(type="RandomErasing", aspect_range=2.0)
    random_erasing = build_from_cfg(cfg, PIPELINES)
    assert random_erasing.aspect_range == (0.5, 2.0)
    ...
    expect_out = np.array(
        [[1, 255, 3, 4], [5, 255, 7, 8], [9, 10, 11, 12]], dtype=np.uint8
    )
    ...
    np.random.seed(0)
    results = construct_toy_data()
    cfg = dict(type="RandomErasing", erase_prob=1.0, mode="rand")
    random_erasing = build_from_cfg(cfg, PIPELINES)
    results = random_erasing(results)
    ...
    results = construct_toy_data()
    cfg = dict(
        type="RandomErasing", erase_prob=1.0, mode="rand", fill_std=(10, 255, 0)
    )
    random_erasing = build_from_cfg(cfg, PIPELINES)
    results = random_erasing(results)
...
def test_color_jitter():
    ...
    transform = dict(
        type="ColorJitter", brightness=0.0, contrast=0.0, saturation=0.0
    )
