...
from pepper.datasets.builder import PIPELINES
...
def construct_toy_data_photometric():
    img = np.array(
        [[0, 128, 255], [1, 127, 254], [2, 129, 253]], dtype=np.uint8
    )
    img = np.stack([img, img, img], axis=-1)
    results = dict()
    # image
...
def test_auto_augment():
    policies = [
        [
            ...
            dict(type="Rotate", angle=30.0, prob=0.6),
        ]
    ]
...
def test_rand_augment():
    ...
    with pytest.raises(AssertionError):
        transform = dict(
            type="RandAugment", policies=[], num_policies=2, magnitude_level=12
        )
        build_from_cfg(transform, PIPELINES)
    with pytest.raises(AssertionError):
        invalid_policies = copy.deepcopy(policies)
    ...
    img_augmented = np.array(
        [[128, 128, 1, 2], [128, 128, 5, 6], [128, 128, 9, 10]], dtype=np.uint8
    )
    img_augmented = np.stack(
        [img_augmented, img_augmented, img_augmented], axis=-1
    )
    ...
    results = construct_toy_data()
    transform = dict(
        type="RandAugment", policies=policies, num_policies=2, magnitude_level=0
    )
    pipeline = build_from_cfg(transform, PIPELINES)
    results = pipeline(results)
...
def test_shear():
    ...
    results = construct_toy_data()
    transform = dict(type="Shear", magnitude=0.0, prob=1.0)
    pipeline = build_from_cfg(transform, PIPELINES)
    results = pipeline(results)
    assert (results["img"] == results["ori_img"]).all()
    ...
    results = construct_toy_data()
    transform = dict(type="Shear", magnitude=0.5, prob=0.0)
    pipeline = build_from_cfg(transform, PIPELINES)
    results = pipeline(results)
    assert (results["img"] == results["ori_img"]).all()
    ...
    results = construct_toy_data()
    transform = dict(
        type="Shear", magnitude=1, pad_val=0, prob=1.0, random_negative_prob=0.0
    )
    pipeline = build_from_cfg(transform, PIPELINES)
    results = pipeline(results)
    sheared_img = np.array(
    ...
    results = pipeline(results)
    sheared_img = np.array(
        [[1, 6, 11, 0], [5, 10, 0, 0], [9, 0, 0, 0]], dtype=np.uint8
    )
    sheared_img = np.stack([sheared_img, sheared_img, sheared_img], axis=-1)
    assert (results["img"] == sheared_img).all()
...
def test_translate():
    ...
    results = construct_toy_data()
    transform = dict(type="Translate", magnitude=0.0, prob=1.0)
    pipeline = build_from_cfg(transform, PIPELINES)
    results = pipeline(results)
    assert (results["img"] == results["ori_img"]).all()
    ...
    results = construct_toy_data()
    transform = dict(type="Translate", magnitude=1.0, prob=0.0)
    pipeline = build_from_cfg(transform, PIPELINES)
    results = pipeline(results)
    assert (results["img"] == results["ori_img"]).all()
    ...
    pipeline = build_from_cfg(transform, PIPELINES)
    results = pipeline(results)
    translated_img = np.array(
        [[0, 0, 1, 2], [0, 0, 5, 6], [0, 0, 9, 10]], dtype=np.uint8
    )
    translated_img = np.stack(
        [translated_img, translated_img, translated_img], axis=-1
    )
    ...
    results = pipeline(results)
    translated_img = np.array(
        [[9, 10, 11, 12], [0, 0, 0, 0], [0, 0, 0, 0]], dtype=np.uint8
    )
    translated_img = np.stack(
        [translated_img, translated_img, translated_img], axis=-1
    )
...
def test_rotate():
    ...
    with pytest.raises(AssertionError):
        transform = dict(type="Rotate", angle=90.0, center=0)
        build_from_cfg(transform, PIPELINES)
    ...
    with pytest.raises(AssertionError):
        transform = dict(type="Rotate", angle=90.0, scale=None)
        build_from_cfg(transform, PIPELINES)

    # test assertion for invalid pad_val
    with pytest.raises(AssertionError):
        transform = dict(type="Rotate", angle=90.0, pad_val=(0, 0))
        build_from_cfg(transform, PIPELINES)

    # test assertion for invalid value of prob
    with pytest.raises(AssertionError):
        transform = dict(type="Rotate", angle=90.0, prob=100)
        build_from_cfg(transform, PIPELINES)
    ...
    results = construct_toy_data()
    transform = dict(type="Rotate", angle=0.0, prob=1.0)
    pipeline = build_from_cfg(transform, PIPELINES)
    results = pipeline(results)
    assert (results["img"] == results["ori_img"]).all()
    ...
    results = construct_toy_data()
    transform = dict(type="Rotate", angle=360.0, prob=1.0)
    pipeline = build_from_cfg(transform, PIPELINES)
    results = pipeline(results)
    assert (results["img"] == results["ori_img"]).all()
    ...
    results = construct_toy_data()
    transform = dict(type="Rotate", angle=90.0, prob=0.0)
    pipeline = build_from_cfg(transform, PIPELINES)
    results = pipeline(results)
    assert (results["img"] == results["ori_img"]).all()
    ...
    results = construct_toy_data()
    transform = dict(
        type="Rotate", angle=30.0, pad_val=0, prob=1.0, random_negative_prob=0.0
    )
    pipeline = build_from_cfg(transform, PIPELINES)
    results = pipeline(results)
    rotated_img = np.array(
        [[5, 2, 2, 0], [9, 6, 7, 4], [0, 11, 11, 8]], dtype=np.uint8
    )
    ...
    results = pipeline(results)
    rotated_img = np.array(
        [[9, 5, 1, 128], [10, 6, 2, 128], [11, 7, 3, 128]], dtype=np.uint8
    )
    rotated_img = np.stack([rotated_img, rotated_img, rotated_img], axis=-1)
    assert (results["img"] == rotated_img).all()
    assert (results["img"] == results["img2"]).all()
    ...
    results = pipeline(results)
    rotated_img = np.array(
        [[4, 8, 12, 0], [3, 7, 11, 0], [2, 6, 10, 0]], dtype=np.uint8
    )
    rotated_img = np.stack([rotated_img, rotated_img, rotated_img], axis=-1)
    assert (results["img"] == rotated_img).all()
    assert (results["img"] == results["img2"]).all()
    ...
    results = pipeline(results)
    rotated_img = np.array(
        [[0, 10, 6, 2], [0, 11, 7, 3], [0, 12, 8, 4]], dtype=np.uint8
    )
    rotated_img = np.stack([rotated_img, rotated_img, rotated_img], axis=-1)
    assert (results["img"] == rotated_img).all()
    assert (results["img"] == results["img2"]).all()
...
def test_auto_contrast():
    ...
    results = construct_toy_data()
    transform = dict(type="AutoContrast", prob=0.0)
    pipeline = build_from_cfg(transform, PIPELINES)
    results = pipeline(results)
    assert (results["img"] == results["ori_img"]).all()
    ...
    results = construct_toy_data()
    transform = dict(type="AutoContrast", prob=1.0)
    pipeline = build_from_cfg(transform, PIPELINES)
    results = pipeline(results)
    ...
    auto_contrasted_img = np.stack(
        [auto_contrasted_img, auto_contrasted_img, auto_contrasted_img], axis=-1
    )
...
def test_invert():
    ...
    results = construct_toy_data()
    transform = dict(type="Invert", prob=0.0)
    pipeline = build_from_cfg(transform, PIPELINES)
    results = pipeline(results)
    assert (results["img"] == results["ori_img"]).all()
    ...
    # test case when prob=1
    results = construct_toy_data()
    transform = dict(type="Invert", prob=1.0)
    pipeline = build_from_cfg(transform, PIPELINES)
    results = pipeline(results)
    ...
    inverted_img = np.stack([inverted_img, inverted_img, inverted_img], axis=-1)
...
def test_equalize(nb_rand_test=100):
    ...
    results = construct_toy_data()
    transform = dict(type="Equalize", prob=0.0)
    pipeline = build_from_cfg(transform, PIPELINES)
    results = pipeline(results)
    assert (results["img"] == results["ori_img"]).all()
    ...
    results = construct_toy_data()
    transform = dict(type="Equalize", prob=1.0)
    pipeline = build_from_cfg(transform, PIPELINES)
    for _ in range(nb_rand_test):
        img = np.clip(
            np.random.normal(0, 1, (256, 256, 3)) * 260, 0, 255
        ).astype(np.uint8)
        results["img"] = img
...
def test_solarize():
    ...
    transform = dict(type="Solarize", thr=128, prob=0.0)
    pipeline = build_from_cfg(transform, PIPELINES)
    results = pipeline(results)
    assert (results["img"] == results["ori_img"]).all()
    ...
    results = construct_toy_data_photometric()
    transform = dict(type="Solarize", thr=256, prob=1.0)
    pipeline = build_from_cfg(transform, PIPELINES)
    results = pipeline(results)
    assert (results["img"] == results["ori_img"]).all()
    ...
    results = construct_toy_data_photometric()
    transform = dict(type="Solarize", thr=128, prob=1.0)
    pipeline = build_from_cfg(transform, PIPELINES)
    results = pipeline(results)
    img_solarized = np.array(
        [[0, 127, 0], [1, 127, 1], [2, 126, 2]], dtype=np.uint8
    )
    img_solarized = np.stack(
        [img_solarized, img_solarized, img_solarized], axis=-1
    )
    ...
    results = construct_toy_data_photometric()
    transform = dict(type="Solarize", thr=100, prob=1.0)
    pipeline = build_from_cfg(transform, PIPELINES)
    results = pipeline(results)
    img_solarized = np.array(
        [[0, 127, 0], [1, 128, 1], [2, 126, 2]], dtype=np.uint8
    )
    img_solarized = np.stack(
        [img_solarized, img_solarized, img_solarized], axis=-1
    )
...
def test_solarize_add():
    ...
    # test case when prob=0, therefore no solarize
    results = construct_toy_data_photometric()
    transform = dict(type="SolarizeAdd", magnitude=100, thr=128, prob=0.0)
    pipeline = build_from_cfg(transform, PIPELINES)
    results = pipeline(results)
    assert (results["img"] == results["ori_img"]).all()
    ...
    results = construct_toy_data_photometric()
    transform = dict(type="SolarizeAdd", magnitude=100, thr=0, prob=1.0)
    pipeline = build_from_cfg(transform, PIPELINES)
    results = pipeline(results)
    assert (results["img"] == results["ori_img"]).all()
    ...
    results = construct_toy_data_photometric()
    transform = dict(type="SolarizeAdd", magnitude=100, thr=128, prob=1.0)
    pipeline = build_from_cfg(transform, PIPELINES)
    results = pipeline(results)
    img_solarized = np.array(
    ...
    results = construct_toy_data_photometric()
    transform = dict(type="SolarizeAdd", magnitude=50, thr=100, prob=1.0)
    pipeline = build_from_cfg(transform, PIPELINES)
    results = pipeline(results)
    img_solarized = np.array(
        [[50, 128, 255], [51, 127, 254], [52, 129, 253]], dtype=np.uint8
    )
    img_solarized = np.stack(
        [img_solarized, img_solarized, img_solarized], axis=-1
    )
...
def test_posterize():
    ...
    results = construct_toy_data_photometric()
    transform = dict(type="Posterize", bits=4, prob=0.0)
    pipeline = build_from_cfg(transform, PIPELINES)
    results = pipeline(results)
    assert (results["img"] == results["ori_img"]).all()
    ...
    results = construct_toy_data_photometric()
    transform = dict(type="Posterize", bits=8, prob=1.0)
    pipeline = build_from_cfg(transform, PIPELINES)
    results = pipeline(results)
    assert (results["img"] == results["ori_img"]).all()
    ...
    results = construct_toy_data_photometric()
    transform = dict(type="Posterize", bits=1, prob=1.0)
    pipeline = build_from_cfg(transform, PIPELINES)
    results = pipeline(results)
    img_posterized = np.array(
        [[0, 128, 128], [0, 0, 128], [0, 128, 128]], dtype=np.uint8
    )
    img_posterized = np.stack(
        [img_posterized, img_posterized, img_posterized], axis=-1
    )
    ...
    results = construct_toy_data_photometric()
    transform = dict(type="Posterize", bits=3, prob=1.0)
    pipeline = build_from_cfg(transform, PIPELINES)
    results = pipeline(results)
    img_posterized = np.array(
        [[0, 128, 224], [0, 96, 224], [0, 128, 224]], dtype=np.uint8
    )
    img_posterized = np.stack(
        [img_posterized, img_posterized, img_posterized], axis=-1
    )
...
def test_contrast(nb_rand_test=100):
    ...
    results = construct_toy_data_photometric()
    transform = dict(type="Contrast", magnitude=0.0, prob=1.0)
    pipeline = build_from_cfg(transform, PIPELINES)
    results = pipeline(results)
    assert (results["img"] == results["ori_img"]).all()
    ...
    results = construct_toy_data_photometric()
    transform = dict(type="Contrast", magnitude=1.0, prob=0.0)
    pipeline = build_from_cfg(transform, PIPELINES)
    results = pipeline(results)
    assert (results["img"] == results["ori_img"]).all()
    ...
    for _ in range(nb_rand_test):
        ...
        pipeline = build_from_cfg(transform, PIPELINES)
        img = np.clip(
            np.random.uniform(0, 1, (256, 256, 3)) * 260, 0, 255
        ).astype(np.uint8)
        results["img"] = img
        results = pipeline(copy.deepcopy(results))
        ...
        np.testing.assert_allclose(
            results["img"], _adjust_contrast(img, 1 + magnitude), rtol=0, atol=1
        )
...
def test_color_transform():
    ...
    results = construct_toy_data_photometric()
    transform = dict(type="ColorTransform", magnitude=0.0, prob=1.0)
    pipeline = build_from_cfg(transform, PIPELINES)
    results = pipeline(results)
    assert (results["img"] == results["ori_img"]).all()
    ...
    results = construct_toy_data_photometric()
    transform = dict(type="ColorTransform", magnitude=1.0, prob=0.0)
    pipeline = build_from_cfg(transform, PIPELINES)
    results = pipeline(results)
    assert (results["img"] == results["ori_img"]).all()
    ...
    results = construct_toy_data_photometric()
    transform = dict(
        type="ColorTransform", magnitude=-1.0, prob=1.0, random_negative_prob=0
    )
    pipeline = build_from_cfg(transform, PIPELINES)
    results = pipeline(results)
    ...
    results = construct_toy_data_photometric()
    transform = dict(
        type="ColorTransform", magnitude=0.5, prob=1.0, random_negative_prob=0
    )
    pipeline = build_from_cfg(transform, PIPELINES)
    results = pipeline(results)
    img_r = np.round(
        np.clip((results["ori_img"] * 0.5 + img_gray * 0.5), 0, 255)
    ).astype(results["ori_img"].dtype)
    ...
    results = construct_toy_data_photometric()
    transform = dict(
        type="ColorTransform", magnitude=0.3, prob=1.0, random_negative_prob=1.0
    )
    pipeline = build_from_cfg(transform, PIPELINES)
    results = pipeline(results)
    img_r = np.round(
        np.clip((results["ori_img"] * 0.7 + img_gray * 0.3), 0, 255)
    ).astype(results["ori_img"].dtype)
    assert (results["img"] == img_r).all()
    assert (results["img"] == results["img2"]).all()
...
def test_brightness(nb_rand_test=100):
    ...
    results = construct_toy_data_photometric()
    transform = dict(type="Brightness", magnitude=0.0, prob=1.0)
    pipeline = build_from_cfg(transform, PIPELINES)
    results = pipeline(results)
    assert (results["img"] == results["ori_img"]).all()
    ...
    results = construct_toy_data_photometric()
    transform = dict(type="Brightness", magnitude=1.0, prob=0.0)
    pipeline = build_from_cfg(transform, PIPELINES)
    results = pipeline(results)
    assert (results["img"] == results["ori_img"]).all()
...
def test_sharpness(nb_rand_test=100):
    ...
    results = construct_toy_data_photometric()
    transform = dict(type="Sharpness", magnitude=0.0, prob=1.0)
    pipeline = build_from_cfg(transform, PIPELINES)
    results = pipeline(results)
    assert (results["img"] == results["ori_img"]).all()
    ...
    results = construct_toy_data_photometric()
    transform = dict(type="Sharpness", magnitude=1.0, prob=0.0)
    pipeline = build_from_cfg(transform, PIPELINES)
    results = pipeline(results)
    assert (results["img"] == results["ori_img"]).all()
...
def test_cutout():
    ...
    results = construct_toy_data()
    transform = dict(type="Cutout", shape=2, prob=0.0)
    pipeline = build_from_cfg(transform, PIPELINES)
    results = pipeline(results)
    assert (results["img"] == results["ori_img"]).all()
    ...
    results = construct_toy_data()
    transform = dict(type="Cutout", shape=0, prob=1.0)
    pipeline = build_from_cfg(transform, PIPELINES)
    results = pipeline(results)
    assert (results["img"] == results["ori_img"]).all()
    ...
    results = construct_toy_data()
    transform = dict(type="Cutout", shape=6, prob=1.0)
    pipeline = build_from_cfg(transform, PIPELINES)
    results = pipeline(results)
    ...
    np.random.seed(0)
    results = construct_toy_data()
    transform = dict(type="Cutout", shape=1, prob=1.0)
    pipeline = build_from_cfg(transform, PIPELINES)
    results = pipeline(results)
    img_cutout = np.array(
        [[1, 2, 3, 4], [5, 128, 7, 8], [9, 10, 11, 12]], dtype=np.uint8
    )
    ...
    np.random.seed(0)
    results = construct_toy_data()
    transform = dict(type="Cutout", shape=(1, 2), pad_val=0, prob=1.0)
    pipeline = build_from_cfg(transform, PIPELINES)
    results = pipeline(results)
    img_cutout = np.array(
        [[1, 2, 3, 4], [5, 0, 0, 8], [9, 10, 11, 12]], dtype=np.uint8
    )
    img_cutout = np.stack([img_cutout, img_cutout, img_cutout], axis=-1)
    assert (results["img"] == img_cutout).all()
