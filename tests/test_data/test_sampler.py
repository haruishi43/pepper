...
"""
- `get_dist_info` should return 0, 1
"""
...
@patch("pepper.datasets.samplers.identity_sampler.no_index", return_value=1)
def test_sampler_builder(_):
    print("argument", _)
    assert build_sampler(None) is None
    dataset = construct_toy_single_label_dataset(1000)[0]
    build_sampler(dict(type="NaiveIdentitySampler", dataset=dataset))
