...
model = dict(
    head=dict(
        ...
        loss_pairwise=[
            dict(type='CircleLoss', margin=0.25, gamma=80, loss_weight=1.0),
        ],
        linear_layer=dict(
            type='CircleSoftmax',
            scale=30,
...
optimizer = dict(
    ...
    paramwise_cfg={
        "head.classifier": dict(lr_mult=1, decay_mult=0.0),
        "head.fc2": dict(lr_mult=1, decay_mult=0.0),  # lr_mult should be 10x
    },
)
...
work_dir = './work_dirs/circleloss_mnist/'
