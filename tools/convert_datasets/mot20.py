#!/usr/bin/env python3

import argparse
import json
import os.path as osp

import cv2
import numpy as np
from tqdm import tqdm

from mmcv.utils import mkdir_or_exist

from tools.convert_datasets.mot_common import (
    crop_person,
    get_frame_paths,
    get_gts,
)

# Globals
DATASET_DIR = "MOT20"
DET_FILE = "{root}/{split}/MOT20-{seq}/det/det.txt"
GT_FILE = "{root}/{split}/MOT20-{seq}/gt/gt.txt"
IMG_DIR = "{root}/{split}/MOT20-{seq}/img1"
SEQUENCES = ("01", "02", "03", "05")


def parse_args():
    parser = argparse.ArgumentParser(description="Convert/format MOT20 dataset")
    parser.add_argument(
        "--root",
        type=str,
        default=f"data/{DATASET_DIR}",
        help="root directory",
    )
    parser.add_argument(
        "-o",
        "--out-dir",
        type=str,
        default="gtPepper",
        help="where to save the processed gts (`gtPepper`)",
    )
    parser.add_argument(
        "--test_mode",
        action="store_true",
    )
    args = parser.parse_args()
    return args


if __name__ == "__main__":

    args = parse_args()

    save_path_tmp = "{root}/{save_root}/{seq}/{pid}_{frame}.jpg"

    assert osp.exists(args.root)

    mkdir_or_exist(osp.join(args.root, args.out_dir))

    # save all images
    total_pids = 0
    metas = []
    for seq in SEQUENCES:

        meta = []

        gt_file = GT_FILE.format(
            root=args.root,
            split="train",
            seq=seq,
        )
        gts = get_gts(gt_file)

        img_dir = IMG_DIR.format(
            root=args.root,
            split="train",
            seq=seq,
        )
        frame_paths = get_frame_paths(img_dir)

        pids = {}
        max_pids = 1

        for frame_path in tqdm(frame_paths):

            frame_img = cv2.imread(frame_path)

            # get frame id (frame starts with 1 and not 0)
            frame = int(osp.split(frame_path)[-1].split(".")[0])

            # filter out
            frame_gts = gts[
                (gts.frame == frame)
                & (gts.is_ped == 1)
                & (
                    gts.vis_ratio > 0.0
                )  # make sure it's not completely occluded
            ]

            for _, person_gt in frame_gts.iterrows():
                # bbox: [x1, y1, w, h]
                bbox = np.array(person_gt[2:6]).astype("i").copy()
                # pid:
                pid = int(person_gt["id"])
                if pid in pids.keys():
                    # seen pid
                    pid = pids[pid]
                else:
                    # new pid
                    pids[pid] = max_pids + total_pids
                    max_pids += 1
                    pid = pids[pid]

                # vis: ratio
                vis_ratio = float(person_gt["vis_ratio"])

                # string_format
                frame_str = str(frame).zfill(6)
                pid_str = str(pid).zfill(6)

                # save crop (convert to xxyxy)
                bbox[2:] += bbox[:2]
                bbox[bbox < 0] = 0

                # create save path for crop
                img_save_path = save_path_tmp.format(
                    root=args.root,
                    save_root=args.out_dir,
                    seq=seq,
                    pid=pid_str,
                    frame=frame_str,
                )

                # crop and save
                if args.test_mode:
                    meta.append(
                        dict(
                            pid=pid,
                            camid=None,
                            img_path=img_save_path,
                            seq=seq,
                            vis_ratio=vis_ratio,
                        )
                    )
                else:
                    mkdir_or_exist(osp.dirname(img_save_path))
                    cropped_img = crop_person(
                        img=frame_img,
                        bbox=bbox,
                        save_path=img_save_path,
                    )

                    if cropped_img is not None:
                        meta.append(
                            dict(
                                pid=pid,
                                camid=None,
                                img_path=img_save_path,
                                seq=seq,
                                vis_ratio=vis_ratio,
                            )
                        )

        num_unique = len(list(pids.keys()))

        total_pids += num_unique
        metas += meta

        # checks
        print("seq", seq, num_unique)
        assert total_pids == max(np.unique(np.array([m["pid"] for m in metas])))

    # do some checks!
    assert total_pids == max(np.unique(np.array([m["pid"] for m in metas])))
    print("total pids", total_pids)

    print("total samples", len(metas))

    # dump metas
    if args.test_mode:
        print(">>> skipped save")
    else:
        save_fp = osp.join(args.root, args.out_dir, "train.json")
        with open(save_fp, "w") as f:
            json.dump(metas, f, indent=4)
