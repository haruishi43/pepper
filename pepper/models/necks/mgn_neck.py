...
class AMGNPooling(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # don't pool for attention branch
        a1 = inputs[3]

        p1_global = self.p1_global_pool(p1)
        p2_global = self.p2_global_pool(p2)
        p3_global = self.p3_global_pool(p3)

        p2_parts = self.p2_part_pool(p2)
        p3_parts = self.p3_part_pool(p3)

        return (p1_global, p2_global, p3_global, p2_parts, p3_parts, a1)
