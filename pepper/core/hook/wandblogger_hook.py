...
class PepperWandbHook(WandbLoggerHook):
    ...
    def before_run(self, runner: BaseRunner):
        ...
        if self.log_evaluation or self.log_checkpoint_metadata:
            ...
            if self.log_evaluation and self.num_eval_images > len(
                self.val_dataset
            ):
    ...
    def after_train_iter(self, runner):
        ...
        if (
            ...
            model_path = osp.join(
                self.ckpt_hook.out_dir, f"iter_{runner.iter+1}.pth"
            )
            self._log_ckpt_as_artifact(model_path, aliases, metadata)
    ...
    def _init_pred_table(self):
        ...
        columns += ["image_name", "image", "ground_truth", "prediction"] + list(
    ...
    def _add_ground_truth(self):
        ...
        for idx in self.eval_image_indexs:
            ...
            self.data_table.add_data(
                image_name, self.wandb.Image(image), CLASSES[gt_label]
            )
